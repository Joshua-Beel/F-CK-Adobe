...
pub struct Note { pub id: String, pub rect: CropBox, pub contents: String, pub kind: AnnotationKind }
...
#[derive(Serialize)]
pub struct AnnotationInfo { pub id: String, pub kind: AnnotationKind, pub page: usize, pub rect: Option<DisplayRect>, pub contents: Option<String> }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
    ...
    for page_id in pages.into_values() {
        ...
        if let Ok(annots) = page.get(b"Annots") {
            ...
            for annotation in annots {
                ...
                if kind == AnnotationKind::Highlight && values(annotation.get(b"QuadPoints").map_err(|error| error.to_string())?)? != quad(bounds) { return Err("The PDF has altered highlight quadrilaterals.".into()); }
                ...
                if stream.content != appearance(bounds, kind) || stream.dict.get(b"Length").is_ok_and(|value| value.as_i64().ok() != Some(stream.content.len() as i64)) { return Err("The PDF has an altered note appearance.".into()); }
                ...
                notes.push(Note { id: note_id, rect: bounds, contents, kind });
            }
        }
        result.push(notes);
    }
...
pub fn write(document: &mut Document, plan: &[PageSpec]) -> Result<(), String> {
    ...
    for spec in plan {
        ...
        for note in &spec.notes {
            let appearance = document.add_object(Stream::new(appearance_dict(note.rect, note.kind), appearance(note.rect, note.kind)));
            ...
            else { annotation.set("QuadPoints", quad(note.rect).into_iter().map(Object::Real).collect::<Vec<_>>()); annotation.set("C", vec![1.into(), 1.into(), 0.into()]); if !note.contents.is_empty() { annotation.set("Contents", encode_text(&note.contents)); } }
...
mod tests {
    ...
    fn highlights_optional_body_caps_history_and_cross_kind_ids_are_consistent() {
        ...
        session.plan[0].notes = (1..=MAX_NOTES).map(|value| Note { id: if value % 2 == 0 { highlight_id(value as u64) } else { id(value as u64) }, kind: if value % 2 == 0 { AnnotationKind::Highlight } else { AnnotationKind::Note }, rect: bounds, contents: "x".into() }).collect(); assert!(validate_plan(&session.plan).is_ok()); session.plan[0].notes.push(Note { id: highlight_id(1001), kind: AnnotationKind::Highlight, rect: bounds, contents: String::new() }); assert!(validate_plan(&session.plan).unwrap_err().contains("1,000"));
        session.plan[0].notes = (1..=128).map(|value| Note { id: if value % 2 == 0 { highlight_id(value) } else { id(value) }, kind: if value % 2 == 0 { AnnotationKind::Highlight } else { AnnotationKind::Note }, rect: bounds, contents: "x".repeat(MAX_TEXT_BYTES) }).collect(); assert!(validate_plan(&session.plan).is_ok()); session.plan[0].notes.push(Note { id: highlight_id(129), kind: AnnotationKind::Highlight, rect: bounds, contents: "x".into() }); assert!(validate_plan(&session.plan).unwrap_err().contains("1 MiB"));
    }
    #[test]
    ...
    fn comments_text_count_total_caps_and_duplicate_ids_are_explicit() {
        ...
        session.plan[0].notes = (1..=MAX_NOTES).map(|value| Note { id: id(value as u64), rect: bounds, contents: "x".into(), kind: AnnotationKind::Note }).collect(); assert!(validate_plan(&session.plan).is_ok());
        session.plan[0].notes.push(Note { id: id(MAX_NOTES as u64 + 1), rect: bounds, contents: "x".into(), kind: AnnotationKind::Note }); assert!(validate_plan(&session.plan).unwrap_err().contains("1,000"));
        session.plan[0].notes = (1..=128).map(|value| Note { id: id(value), rect: bounds, contents: "x".repeat(MAX_TEXT_BYTES), kind: AnnotationKind::Note }).collect(); assert!(validate_plan(&session.plan).is_ok());
        session.plan[0].notes.push(Note { id: id(129), rect: bounds, contents: "x".into(), kind: AnnotationKind::Note }); assert!(validate_plan(&session.plan).unwrap_err().contains("1 MiB"));
        session.plan[0].notes = vec![Note { id: id(1), rect: bounds, contents: "x".into(), kind: AnnotationKind::Note }; 2]; assert!(validate_plan(&session.plan).unwrap_err().contains("duplicate"));
