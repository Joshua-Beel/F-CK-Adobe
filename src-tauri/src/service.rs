                ...
                match request {
                    ...
                    Request::FormFields(id, revision, reply) => {
                        ...
                        let result = (|| {
                            ...
                            if query.status=="supported" && query.fields.iter().any(|field|field.checked.is_some()) {
                                let values=documents.get(&id).and_then(|document|checked_engine_form_values(document,&query.fields));
                                if values.as_ref().is_none_or(|values|!filled_values_agree(&query.fields,values)) {query.status="unsupported";query.reason=Some("The checkbox states disagree with the rendering engine.".into());query.fields.clear();}
                            }
                            Ok(query)
                        })();
                        let _ = reply.send(result);
...
mod tests {
    ...
    fn forms_empty_values_require_an_actual_named_engine_field() {
        let expected = vec![crate::forms::FormField {field_id:"opaque".into(),name:"Name".into(),page:0,value:String::new(),max_length:None,checked:None,on_state:None}];
