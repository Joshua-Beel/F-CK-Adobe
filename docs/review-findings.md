...
- Installed-app password/clipboard/restart behavior, native print output and driver failures, the signed installer upgrade from 0.2.0, interrupted-install recovery and relaunch still require end-to-end verification.
...
Combined automated gate: 59 frontend/release tests, 32 native tests, and the production frontend build passed. These checks exercise the current source; the 0.2.4 draft predates these changes.
