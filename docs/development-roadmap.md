...
| Worker backlog measurement, exact-version MPL source availability, then installed printing and update verification | Existing worker lifecycle and a current signed installer | Measure retained and queued work, and verify source-availability packaging for the five identified MPL components. Worker-reply cleanup covers raw worker-to-service receivers but not frontend acknowledgment. Then inspect Print to PDF output and printer cancellation; install and upgrade from 0.2.0, preserve documents/settings, relaunch and exercise interruption/retry. The existing desktop-control blocker must change before repeating attempts. |
