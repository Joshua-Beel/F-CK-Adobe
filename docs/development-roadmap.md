...
| Adversarial print-reply cleanup, then installed printing and update verification | Existing print implementation and a current signed installer | Reproduce reply cleanup under cancellation or receiver loss, then inspect Print to PDF output and printer cancellation; install and upgrade from 0.2.0, preserve documents/settings, relaunch and exercise interruption/retry. The existing desktop-control blocker must change before repeating attempts. |
