...
The installer, signed updater, and release CI are implemented. The 0.2.4 draft installer and packaged application have verified timestamped publisher signatures; installation and the full upgrade/relaunch remain unverified. The [dependency inventory](license-inventory.md) records generated notice packaging and remaining source-availability/license checks. Automated desktop UI tests, new installer notice verification, and a 200-file real corpus remain outstanding. Synthetic tests and manual UI checks are recorded separately in phase status. The current build must not be used as a redaction, document-signing or PDF/A-conformance tool.
