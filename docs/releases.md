...
Publish publisher signing as a new version; do not replace the already published 0.2.0 installer, because its Tauri signature and manifest identify those exact bytes. Version 0.2.6 is tagged and its workflow is running. Draft assets, installation, and upgrade checks remain pending.
...
The immutable `v0.2.5` tag started [GitHub Actions run 35250005735](https://github.com/Joshua-Beel/Smacrobat/actions/runs/35250005735). It stopped during npm tests because the clean runner had not fetched the locked Cargo registry required by offline dependency-notice checks. No native test, signing, installer creation, asset upload, or draft-release step ran, so no 0.2.5 signed artifact exists. The `v0.2.6` tag started [GitHub Actions run 35250878119](https://github.com/Joshua-Beel/Smacrobat/actions/runs/35250878119), which fetches that locked Windows registry before the unchanged fail-closed offline check. Its locked fetch and full npm tests passed; native Cargo remains in progress. Build and release evidence must be recorded only after the workflow succeeds. Native save dialogs, installation, and upgrade/relaunch remain separate desktop checks.
